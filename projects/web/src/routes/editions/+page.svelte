<script lang="ts">
	...
	async function toggleWork(workId: number) {
		...
		if (!tocCache[workId]) {
			...
			try {
				...
				console.error('[editions] failed to load TOC:', err);
			} finally {
				tocLoading = false;
			}
		}
	...
	function gotoScene(workId: number, act: number, scene: number) {
		goto(`/text/${workId}/${act}/${scene}`);
	}

	function continueReading(workId: number) {
		const pos = readingPosition.get(workId);
		if (pos) goto(`/text/${workId}/${pos.act}/${pos.scene}`);
	}
...
{#snippet workToc(workId: number)}
	{@const saved = readingPosition.get(workId)}
	<div class="work-toc">
		{#if saved}
			<button class="continue-btn" onclick={() => continueReading(workId)}>
				Continue reading &mdash; {formatPosition(workId)}
			</button>
		{/if}
		{#if tocLoading && !tocCache[workId]}
			<div class="toc-loading">Loading&hellip;</div>
		{:else if tocCache[workId]}
			{@const acts = tocByAct(tocCache[workId])}
					...
					<ul class="toc-scene-list">
						{#each scenes as sc (sc.scene)}
							<li>
								<button
									class="toc-scene-btn"
									onclick={() => gotoScene(workId, sc.act, sc.scene)}
								>
									{#if actNum === 0}
...
<div class="editions-page">
	...
	{#each ['tragedy', 'comedy', 'history'] as workType (workType)}
		{@const works = playsByType().get(workType) ?? []}
		{#if works.length > 0}
	...
	{#if data.works.poetry.length > 0}
		<section class="work-group">
			<h2 class="group-title">Poetry & Other</h2>
			<ul class="work-list">
